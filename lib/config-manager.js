...
class ConfigManager {
  ...
  async loadAll() {
    ...
    try {
      ...
      // Merge vaults into obsidian config for unified access
  ...
  applyEnvVarOverrides() {
    ...
    if (process.env.OBSIDIAN_API_KEY) {
      this.setNestedValue(this.configs.obsidian, 'api.localRestApi.authentication.apiKey', process.env.OBSIDIAN_API_KEY);
    }
  ...
  validate() {
    ...
    if (!this.configs.obsidian.vaults || this.configs.obsidian.vaults.length === 0) {
      errors.push('No vaults configured');
    }
